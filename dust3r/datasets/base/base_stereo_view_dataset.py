...
class BaseStereoViewDataset (EasyDataset):
    ...
    def _crop_resize_if_necessary(self, image, depthmap, intrinsics, resolution, 
                                  rng=None, info=None, offset_factor=0.5, mask=None, normalmap=None):
        """ This function:
        ...
        crop_bbox = (l, t, r, b)
        ret_crop_list = cropping.crop_image_depthmap(image, depthmap, intrinsics, crop_bbox, mask=mask, normalmap=normalmap)
        image, depthmap, intrinsics = ret_crop_list[:3]
        if mask is not None:
            mask = ret_crop_list[3]
        if normalmap is not None:
            normalmap = ret_crop_list[4]
        ...
        ret_rescale_list = cropping.rescale_image_depthmap(image, depthmap, intrinsics, target_resolution, mask=mask, normalmap=normalmap)
        image, depthmap, intrinsics = ret_rescale_list[:3]
        if mask is not None:
            mask = ret_rescale_list[3]
        if normalmap is not None:
            normalmap = ret_rescale_list[4]
        ...
        if normalmap is not None:
            normalmap = ret_crop_list[4]
        return image, depthmap, intrinsics2, mask, normalmap
...
def transpose_to_landscape(view):
    ...
    if width < height:
        ...
        if "mask" in view:
            assert view['mask'].shape == (height, width)
            view['mask'] = view['mask'].swapaxes(0, 1)
