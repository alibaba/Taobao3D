...
// float4 _Packed_Params0; // uint numKeys, int numBlocksPerThreadGroup, uint numThreadGroups, uint numThreadGroupsWithAdditionalBlocks
// float4 _Packed_Params1; // uint numReduceThreadgroupPerBin, uint numScanValues, uint shift, uint padding

/*
uint FfxNumKeys() { return asuint(_Packed_Params0.x); }
int FfxNumBlocksPerThreadGroup() { return asint(_Packed_Params0.y); }
uint FfxNumThreadGroups() { return asuint(_Packed_Params0.z); }
uint FfxNumThreadGroupsWithAdditionalBlocks() { return asuint(_Packed_Params0.w); }
uint FfxNumReduceThreadgroupPerBin() { return asuint(_Packed_Params1.x); }
uint FfxNumScanValues() { return asuint(_Packed_Params1.y); }
uint FfxShiftBit() { return asuint(_Packed_Params1.z); }
*/
...
StructuredBuffer<uint> sort_params : register(t1); // uint[8] type

uint FfxNumKeys() { return sort_params[0]; }
int FfxNumBlocksPerThreadGroup() { return sort_params[1]; }
uint FfxNumThreadGroups() { return sort_params[2]; }
uint FfxNumThreadGroupsWithAdditionalBlocks() { return sort_params[3]; }
uint FfxNumReduceThreadgroupPerBin() { return sort_params[4]; }
uint FfxNumScanValues() { return sort_params[5]; }
uint FfxShiftBit() { return asuint(_Packed_Params1.z); }

// -------- Read/Write buffers

RWStructuredBuffer<uint> rw_source_keys : register(u2);
RWStructuredBuffer<uint> rw_dest_keys : register(u3);
RWStructuredBuffer<uint> rw_source_payloads : register(u4);
RWStructuredBuffer<uint> rw_dest_payloads : register(u5);
RWStructuredBuffer<uint> rw_sum_table : register(u6);
RWStructuredBuffer<uint> rw_reduce_table : register(u7);
RWStructuredBuffer<uint> rw_scan_source : register(u8);
RWStructuredBuffer<uint> rw_scan_dest : register(u9);
RWStructuredBuffer<uint> rw_scan_scratch : register(u10);
...
void ffxParallelSortCountUInt(uint localID, uint groupID, uint ShiftBit)
{
    ...
    if (groupID >= NumThreadGroups)
    {
        // we dispatch more thread groups than needed when doing partial sorting without indirect dispatch (not supported on some platforms)
        return;
    }
...
void ffxParallelSortReduceCount(uint localID, uint groupID)
{
    ...
    if (groupID >= NumReduceThreadgroupPerBin * FFX_PARALLELSORT_SORT_BIN_COUNT)
    {
        // we dispatch more thread groups than needed when doing partial sorting without indirect dispatch (not supported on some platforms)
        return;
    }
...
{
    if (GroupID >= FfxNumReduceThreadgroupPerBin() * FFX_PARALLELSORT_SORT_BIN_COUNT)
    {
        // we dispatch more thread groups than needed when doing partial sorting without indirect dispatch (not supported on some platforms)
        return;
    }
...
StructuredBuffer<uint> _SortParams : register(t1); // uint[8] type
StructuredBuffer<uint> _SrcUintBuffer : register(t2);
RWStructuredBuffer<uint> _DstUintBuffer : register(u9);

[numthreads(256, 1, 1)]
void CopyUintBufferIndirect(uint3 id : SV_DispatchThreadID)
{
    uint count = _SortParams[0];
    uint idx = id.x;
    if (idx < count)
    {
        _DstUintBuffer[idx] = _SrcUintBuffer[idx];
    }
}