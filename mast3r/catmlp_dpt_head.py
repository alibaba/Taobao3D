...
from dust3r.heads.postprocess import reg_dense_normal, reg_dense_kappa  # noqa
...
def postprocess(out, depth_mode, conf_mode, desc_dim=None, desc_mode='norm', desc_conf=False, desc_conf_mode=None):
    ...
    if conf_mode is not None:
        ...
        # res['conf'] = torch.ones_like(fmap[..., 3]) * 2.0
...
# only normal in one DPT Head
def postprocess_normal(out, normal_mode):
    fmap = out.permute(0, 2, 3, 1)  # B,H,W,D
    res = dict(normal=reg_dense_normal(fmap[..., 0:3], mode=normal_mode))
    if "kappa" in normal_mode:
        res['kappa'] = reg_dense_kappa(fmap[..., 3], mode=normal_mode)
    return res

def postprocess_semantic(out, normal_mode):
    # out = out.permute(0, 2, 3, 1) # B,H,W,D
    res = dict(seg_prob=out)
    return res
...
class Cat_MLP_LocalFeatures_DPT_Pts3d(PixelwiseTaskWithDPT):
    ...
    def __init__(self, net, has_conf=False, local_feat_dim=16, hidden_dim_factor=4., hooks_idx=None, dim_tokens=None,
        ...
        self.desc_conf = net.desc_conf  # independent confs for 3D regr and descs
        ...
        self.head_local_features = Mlp(in_features=idim,
                                       ...
                                       out_features=(self.local_feat_dim + self.desc_conf) * self.patch_size**2)
    ...
    def forward(self, decout, img_shape):
        ...
        if self.postprocess:
            out = self.postprocess(out,
                                   ...
                                   desc_conf=self.desc_conf,
                                   desc_conf_mode=self.desc_conf_mode)
        return out

def mast3r_head_factory(head_type, output_mode, net, has_conf=False, has_mask=False):
    ...
    if head_type == 'catmlp+dpt' and output_mode.startswith('pts3d+desc'):
        ...
        return Cat_MLP_LocalFeatures_DPT_Pts3d(net, local_feat_dim=local_feat_dim, has_conf=has_conf,
                                               num_channels=out_nchan + has_conf + has_mask,
    ...
    elif head_type == 'dpt' and output_mode.startswith('normal'):
        assert net.dec_depth > 9
        l2 = net.dec_depth
        feature_dim = 256
        last_dim = feature_dim//2
        ...
        ed = net.enc_embed_dim
        dd = net.dec_embed_dim
        if "kappa" in output_mode:
            use_kappa = True
        else:
            use_kappa = False
        return PixelwiseTaskWithDPT(num_channels=out_nchan + use_kappa,
                                    feature_dim=feature_dim,
                                    last_dim=last_dim,
                                    hooks_idx=[0, l2*2//4, l2*3//4, l2],
                                    dim_tokens=[ed, dd, dd, dd],
                                    postprocess=postprocess_normal,
                                    # depth_mode=("linear", -float("inf"), float("inf")),
                                    # conf_mode=("not_used", -float("inf"), float("inf")),
                                    normal_mode=net.normal_mode,
                                    head_type='regression')
    elif head_type == 'dpt' and output_mode.startswith('semantic'):
        assert net.dec_depth > 9
        l2 = net.dec_depth
        feature_dim = 256
        last_dim = feature_dim//2
        ...
        return PixelwiseTaskWithDPT(num_channels=out_nchan,
                                    ...
                                    hooks_idx=[0, l2*2//4, l2*3//4, l2],
                                    dim_tokens=[ed, dd, dd, dd],
                                    postprocess=postprocess_semantic,
                                    normal_mode=net.normal_mode,
                                    head_type='regression')
    else:
