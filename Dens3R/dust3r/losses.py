...
class ConfLoss (MultiLoss):
    ...
    def get_conf_mask(self, gt1):
        '''
        Only apply conf loss on ACENNR/ABO/GObjaverse
        Maybe useless: TODO
        '''
        datatype_list = gt1['dataset']
        need_conf_mask = torch.tensor([False] * len(datatype_list)).to(gt1['pts3d'].device)
        for datatype in datatype_list:
            if datatype in ["AceNNRRaw", "ABO", "GObjaverse"]:
                need_conf_mask[datatype_list.index(datatype)] = True
        return need_conf_mask
    ...
    def compute_loss(self, gt1, gt2, pred1, pred2, **kw):
        ...
        count_loss = conf_loss1 + conf_loss2
        
        # if "normal" in pred1:
        #     count_loss = count_loss + details["normal_loss_count"] * 0.1

        return count_loss, dict(conf_loss_1=float(conf_loss1), conf_loss2=float(conf_loss2), **details)
