...
from dust3r.heads.postprocess import postprocess_mask
...
class DPTOutputAdapter_fix(DPTOutputAdapter):
    ...
    def forward(self, encoder_tokens: List[torch.Tensor], image_size=None):
        ...
        # fxz: interpolate layer to original patch_size
        if layers[0].shape[1] != int(N_H*N_W):
            layers = [nn.functional.interpolate(rearrange(l, 'b (bh bw) c -> b c bh bw', bh=self.atm_size[0], bw=self.atm_size[1]), size=(N_H, N_W), 
                                                mode='bicubic').permute(0, 2, 3, 1).reshape(l.shape[0], int(N_H*N_W), -1) for l in layers]
...
class PixelwiseTaskWithDPT(nn.Module):
    ...
    def __init__(self, *, n_cls_token=0, hooks_idx=None, dim_tokens=None,
                 ...
                 depth_mode=None, conf_mode=None, normal_mode=None, **kwargs):
    ...
    def forward(self, x, img_info):
        ...
        if self.postprocess:
            if self.normal_mode is not None:
                out = self.postprocess(out, self.normal_mode)
            else:
