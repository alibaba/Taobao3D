#ifndef GAUSSIAN_SPLATTING
#define GAUSSIAN_SPLATTING

#include "gaussianDataTypes.hlsl"

uint GetSplatCount() { return asuint(_SplatProp.x); }

uint GetSplatFormat() { return asuint(_SplatProp.y); }

uint GetSHOrder() { return asuint(_SplatProp.z); }

uint GetVertexStride() { return asuint(_SplatProp.w); }

uint UnpackByteAsUint(ByteAddressBuffer buffer, uint byteIndex)
{
...
TriangleTransform GetTriangleTransform(int triIdx, float3 splatPos)
{
	TriangleTransform res = (TriangleTransform)0;

    int mid0 = _MeshIndice[3 * triIdx];
    int mid1 = _MeshIndice[3 * triIdx + 1];
    int mid2 = _MeshIndice[3 * triIdx + 2];
    ...
    res.scale_factor = sqrt(length(cross_res));

    float3 normal = cross(e1, e2);
    float3 e1n = e1 / (length(e1) + 1.e-12f);
    float3 normn = normal / (length(normal) + 1.e-12f);
    float3 axn = cross(e1n, normal);
    axn = axn / (length(axn) + 1.e-12f);

    float3x3 rotation = float3x3(
        e1n.x, e1n.y, e1n.z,
        normn.x, normn.y, normn.z,
        axn.x, axn.y, axn.z);
    res.rotation = transpose(rotation);
    res.origin = v0 * u + v1 * v + v2 * w - (splatPos.y * res.scale_factor) * normn;
    res.face_shadow = 1.0;

    float p0 = _PoseShadowCompensation[mid2];
    float p1 = _PoseShadowCompensation[mid1];
    float p2 = _PoseShadowCompensation[mid0];
    res.face_shadow *= u * p0 + v * p1 + w * p2 + 1.0f;

    float3 e02 = v2 - v0;
    float3 cross_product = cross(e1, e02);
    float cross_product_norm = length(cross_product);
    float3 v3 = v0 + cross_product / sqrt(cross_product_norm);
    float3 cnormal = cross_product / (cross_product_norm + 1.e-12f);
    float3 e03 = v3 - v0;
    float3x3 jacobianE = float3x3(e1.x, e02.x, e03.x,
                                  e1.y, e02.y, e03.y,
                                  e1.z, e02.z, e03.z);
    float3x3 trans = float3x3(0,0,1,1,0,0,0,1,0);
    res.jacobian = mul(jacobianE, trans);
...
SplatData LoadSplatData(uint idx, uint shOrder = 3)
{
    ...
    uint scaleFmt = (GetSplatFormat() >> 8) & 0xFF;
    uint shFormat = (GetSplatFormat() >> 16) & 0xFF;
...
float4 CalcCovariance2D(float3 worldPos, float3 cov3d0, float3 cov3d1, float4x4 matrixV, float4x4 matrixP, float4 screenParams, float kernel_size = 0.1f)
{
	...
	return float4(cov._m00, cov._m01, cov._m11, coef);
}
